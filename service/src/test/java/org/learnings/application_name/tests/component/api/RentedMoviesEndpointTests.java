...
public class RentedMoviesEndpointTests {
    ...
    private final List<RentedMovie> dataSource = List.of(
            new RentedMovie(clientUUID, firstMovieUUID,
                    3, Date.from(Instant.parse("2024-02-01T08:15:24.00Z"))),
            new RentedMovie(clientUUID, UUID.fromString("f9734631-6833-4885-93c5-dd41679fc907"),
                    2, Date.from(Instant.now().minus(Duration.ofDays(30)))),
            new RentedMovie(clientUUID, UUID.fromString("f9734631-6833-4885-93c5-dd41679fc906"),
                    1, Date.from(Instant.now().minus(Duration.ofDays(3))))
    );

    @Test
    void getAllRentedMoviesOfClient() throws Exception {
        when(repository.findByClientID(clientUUID)).thenReturn(dataSource);
    ...
    void getRentedMovieForClient_whenRentedMovieExists() throws Exception {
        when(repository.findByClientIDAndMovieID(clientUUID, firstMovieUUID)).thenReturn(dataSource.get(0));
    ...
    void getRentedMovieForClient_whenNoRentedMovieWithThisID() throws Exception {
        when(repository.findByClientIDAndMovieID(clientUUID, firstMovieUUID)).thenReturn(null);
